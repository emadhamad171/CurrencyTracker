...
const processAlert = async (alert, currentRates) => {
    try {
        ...
        const currencyRates = currentRates.find(rate =>
            rate.currency.toLowerCase() === alert.currency.toLowerCase() &&
            rate.baseCurrency.toLowerCase() === alert.baseCurrency.toLowerCase()
        );

        if (!currencyRates) {
            logger.warn(`No rate found for currency ${alert.currency}/${alert.baseCurrency}`);
            return;
        }
        ...
        if (alert.direction === 'above' && currencyRates.sell >= alert.threshold) {
            isTriggered = true;
        } else if (alert.direction === 'below' && currencyRates.buy <= alert.threshold) {
            isTriggered = true;
        }
        ...
        if (isTriggered) {
            ...
            await db.collection('alertHistory').add({
                ...
                currentRate: alert.direction === 'above' ? currencyRates.sell : currencyRates.buy,
...
const sendPushNotification = async (token, alert, currencyRates) => {
    try {
        const rate = alert.direction === 'above' ? currencyRates.sell : currencyRates.buy;
        const directionText = alert.direction === 'above' ? 'превысил' : 'упал ниже';
