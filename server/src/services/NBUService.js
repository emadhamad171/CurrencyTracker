// server/src/services/NBUService.js
...
class NBUService {
    ...
    async getNBURates(baseCurrency = 'UAH', currencies = ['USD', 'EUR']) {
        try {
            ...
            const response = await this.callMinFinAPI(`${MINFIN_API_BASE}/nbu/${MINFIN_API_KEY}`);

            if (!response || !response.data) {
                throw new Error('Invalid response from NBU API');
            }
            const rates = this.formatNBURates(response.data, currencies, baseCurrency);
    ...
    async getBankRates(baseCurrency = 'UAH', currencies = ['USD', 'EUR']) {
        try {
            ...
            const bankRatesPromises = currencies.map(currency =>
                this.callMinFinAPI(`${MINFIN_API_BASE}/exchrates/${MINFIN_API_KEY}/${currency}`)
            );
            ...
            responses.forEach((response, index) => {
                if (response && response.data) {
                    ...
                    const bankRates = this.formatBankRates(response.data, currencyCode, baseCurrency);
    ...
    async getBlackMarketRates(baseCurrency = 'UAH', currencies = ['USD', 'EUR']) {
        try {
            ...
            const response = await this.callMinFinAPI(`${MINFIN_API_BASE}/auction/info/${MINFIN_API_KEY}`);

            if (!response || !response.data) {
                throw new Error('Invalid response from Black Market API');
            }

            const rates = this.formatBlackMarketRates(response.data, currencies, baseCurrency);
    ...
    async getHistoricalRates(currency = 'usd', period = 'month') {
        try {
            ...
            const cachedData = await getCachedData(cacheKey);
            // if (cachedData) {
            //     logger.info('Returning cached historical rates');
            //     return cachedData;
            // }

                const response = await this.callMinFinAPI(`https://minfin.com.ua/api/currency/rates/nbu/${currency}/avg?period=${period}`)
    ...
    formatBankRates(data, currency, baseCurrency) {
        ...
        data.data.forEach(bank => {
            if (bank.slug === 'ukrsibbank' || bank.slug === 'monobank' || bank?.slug === 'privatbank') {
                const rateData = bank.card || bank.cash;

                if (rateData) {
                    result.push({
                        source: bank.slug === 'ukrsibbank' ? 'UkrsibBank' : bank?.slug === "privatbank" ? "PrivatBank" :"Monobank",
                        currency: currency.toUpperCase(),
                        baseCurrency,
